...
export default function Register(){
  ...
  const handleSubmit = async (e)=>{
    ...
    // If signUp requires email confirmation, user may be in "pending" state.
    // After sign up, create/update profile via server-side endpoint using service role key.
    try{
  ...
  return (
    <div className="max-w-md mx-auto p-6">
      <h1 className="text-2xl font-playfair mb-4">Register</h1>
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded shadow">
        ...
        <button disabled={loading} className="bg-rose-300 text-rose-900 px-4 py-2 rounded">Register</button>
      </form>
    </div>
  )
}
